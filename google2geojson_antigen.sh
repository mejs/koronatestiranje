...
g_json=$(curl -s -stdout 'https://spreadsheets.google.com/feeds/list/1OAdAQKxZ2kn-_oAX23uBmhMEvHY5w-j7JCwCaPazVTM/6/public/values?alt=json')
...
echo $output | jq '.' > $destFolder"korona_antigen.json"
sed -i "s/\\\\\\\/\\\/g" korona_antigen.json
sed -i 's/"{\\"color/\{"color/g' korona_antigen.json
sed -i 's/\\"\\}/\"}/g' korona_antigen.json
sed -i 's/"{"c/\{"c/g' korona_antigen.json
sed -i 's/or\\":\\"/\lor":"/g' korona_antigen.json
sed -i 's/\\",\\"/\","/g' korona_antigen.json
sed -i 's/Class\\":\\"Drop","iconUrl\\":\\/\Class":"Drop","iconUrl":/g' korona_antigen.json
sed -i 's/png\\}"/\png"}/g' korona_antigen.json
