...
destFolder="/home/$me/github/koronatestiranje/test/"
...
g_json=$(curl -s -stdout 'https://spreadsheets.google.com/feeds/list/1yoJMipemnJuWLKIKLk-nBBcf5FeTrh7IftJwvENHem0/5/public/values?alt=json')
...
echo $output | jq '.' > $destFolder"korona-testDE.json"
sed -i "s/\\\\\\\/\\\/g" korona-testDE.json
sed -i 's/"{\\"color/\{"color/g' korona-testDE.json
sed -i 's/\\"\\}/\"}/g' korona-testDE.json
sed -i 's/"{"c/\{"c/g' korona-testDE.json
sed -i 's/or\\":\\"/\lor":"/g' korona-testDE.json
sed -i 's/\\",\\"/\","/g' korona-testDE.json
sed -i 's/Class\\":\\"Drop","iconUrl\\":\\/\Class":"Drop","iconUrl":/g' korona-testDE.json
sed -i 's/png\\}"/\png"}/g' korona-testDE.json
