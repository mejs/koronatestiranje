...
g_json=$(curl -s -stdout 'https://spreadsheets.google.com/feeds/list/1OAdAQKxZ2kn-_oAX23uBmhMEvHY5w-j7JCwCaPazVTM/4/public/values?alt=json')
...
echo $output | jq '.' > $destFolder"koronaDE.json"
sed -i "s/\\\\\\\/\\\/g" koronaDE.json
sed -i 's/"{\\"color/\{"color/g' koronaDE.json
sed -i 's/\\"\\}/\"}/g' koronaDE.json
sed -i 's/"{"c/\{"c/g' koronaDE.json
sed -i 's/or\\":\\"/\lor":"/g' koronaDE.json
sed -i 's/\\",\\"/\","/g' koronaDE.json
sed -i 's/Class\\":\\"Drop","iconUrl\\":\\/\Class":"Drop","iconUrl":/g' koronaDE.json
sed -i 's/png\\}"/\png"}/g' koronaDE.json
